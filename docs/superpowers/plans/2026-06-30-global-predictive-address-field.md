...
Status: complete in remote commit `a76977b`. The implemented presets are
`gpaf-shadow-v1` and `gpaf-retrieval-v1`; structural-call roles are part of
retrieval mode and do not need a separate preset flag. 10M validation shows
retrieval is mechanically safe and neutral at eval-NLL noise level, with positive
frozen ablation signal but no accepted net quality claim yet.
...
Presets added:
- `gpaf-shadow-v1` — shadow role-key observation, no prediction change.
- `gpaf-retrieval-v1` — full bounded candidate retrieval + structural call roles.

Structural-call roles are an implicit part of retrieval mode, so
`gpaf-structural-call-v1` is not used as a separate preset. Verified:
`python tests/test_presets.py` passes.

- [x] **Step 2: Add experiment reporting fields**

Corpus runners already emit GPAF config, role-key counts, slot lifecycle counts,
source-quota diagnostics, frozen slot ablation and model byte split through the
existing token experiment JSON output. No runner changes are required for this
Task 6 state.

- [x] **Step 3: Update canonical docs after evidence exists**

`DESIGN_NOTES.md`: GPAF section updated with 10M validation summary.
...
Remote Task 6 evidence reports 21/21 CTest passes and `python tests/test_presets.py`
passes. Local verification in this checkout re-runs the preset test and targeted
API/Python tests after reconciliation.

- [x] **Step 5: Commit — final Task 6 commit**

```bash
git add python/sbm_presets.py tests/test_presets.py DESIGN_NOTES.md PROGRESS.md RESEARCH_LOG.md docs/superpowers/plans/2026-06-30-global-predictive-address-field.md
git commit -m "docs: align GPAF Task 6 with 10M validation"
```
